...
export const LandingPage = ({ onEnterPortal, onGoToLogin }) => {
  ...
  const annualSubscription = numResidents * SUBSCRIPTION_PER_RESIDENT_PER_MONTH * 12;
  ...
  const openDemoFromPricing = () => {
    ...
    setDemoForm((prev) => ({
      ...
      notes: `Interested in VAMS bundle. First-year estimate: ${formatPrice(firstYearTotal)}.`
    }));
    setIsDemoOpen(true);
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 text-white overflow-hidden">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-slate-900/95 backdrop-blur-xl border-t border-white/10 animate-slide-down">
            <div className="px-4 py-6 space-y-4">
              ...
              <Button
                onClick={() => { setMobileMenuOpen(false); openDemoBlank('mobile'); }}
                variant="outline"
        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-gray-300 max-w-3xl mx-auto mb-8">
              Transparent bundle with a one-time hardware setup and an all-inclusive annual software subscription.
            </p>
          </div>
          ...
          <div className="space-y-12">
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-3xl p-8 border border-white/20">
              <h3 className="text-2xl font-bold text-white mb-2">1. One-Time Hardware & Setup Fee</h3>
              <p className="text-gray-300 mb-6">Upfront cost for physical hardware and professional onboarding.</p>
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <h4 className="text-lg font-semibold text-white mb-3">Items Included</h4>
                ...
                <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
                  <div className="text-sm text-gray-400 mb-2">Cost per tablet</div>
                  <div className="text-4xl font-bold text-white mb-4">{formatPrice(HARDWARE_COST_PER_TABLET)}</div>
                  <div className="space-y-3">
                    <label className="block text-sm text-gray-300">Number of tablets (min 1)</label>
                    <Input
                      type="number"
                      min={1}
                      value={numTablets}
                      onChange={(e) => setNumTablets(Math.max(1, Number(e.target.value)))}
                      className="bg-white/10 border-white/20 text-white"
                    />
                    <div className="text-gray-300 mt-2">One-time total: <span className="font-semibold text-white">{formatPrice(oneTimeHardware)}</span></div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-3xl p-8 border border-white/20">
              <h3 className="text-2xl font-bold text-white mb-2">2. Annual All-Inclusive Subscription</h3>
              <p className="text-gray-300 mb-6">Single annual fee covering software access and all operational costs, including data.</p>
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                ...
                <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
                  <div className="text-sm text-gray-400 mb-2">Price per resident per month</div>
                  <div className="text-4xl font-bold text-white mb-4">{formatPrice(SUBSCRIPTION_PER_RESIDENT_PER_MONTH)}<span className="text-lg text-gray-300 font-normal">/month</span></div>
                  <div className="space-y-3">
                    <label className="block text-sm text-gray-300">Number of residents (min 30)</label>
                    <Input
                      type="number"
                      min={30}
                      value={numResidents}
                      onChange={(e) => setNumResidents(Math.max(30, Number(e.target.value)))}
                      className="bg-white/10 border-white/20 text-white"
                    />
                    <div className="text-gray-300 mt-2">Annual subscription: <span className="font-semibold text-white">{formatPrice(annualSubscription)}</span></div>
                    <div className="text-sm text-gray-400">Calculation: ({formatPrice(SUBSCRIPTION_PER_RESIDENT_PER_MONTH)} x residents) x 12</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-blue-600/20 to-purple-600/20 backdrop-blur-sm rounded-3xl p-8 border border-blue-400/30">
              <h3 className="text-2xl font-bold text-white mb-4">Total Cost of the Whole Bundle (Currently)</h3>
              <p className="text-gray-300 mb-6">Example for <span className="font-semibold text-white">{numResidents}</span> residents and <span className="font-semibold text-white">{numTablets}</span> tablet{numTablets === 1 ? '' : 's'} for a full year.</p>
              <div className="grid md:grid-cols-3 gap-6">
                <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
                  <div className="text-sm text-gray-400 mb-1">One-Time Hardware & Setup Fee</div>
                  <div className="text-3xl font-bold text-white">{formatPrice(oneTimeHardware)}</div>
                </div>
                <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
                  <div className="text-sm text-gray-400 mb-1">Annual All-Inclusive SaaS Subscription</div>
                  <div className="text-3xl font-bold text-white">{formatPrice(annualSubscription)}</div>
                </div>
                <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
                  <div className="text-sm text-gray-400 mb-1">Total First-Year Cost</div>
                  <div className="text-3xl font-bold text-white">{formatPrice(firstYearTotal)}</div>
                </div>
              </div>
              <div className="text-center text-gray-300 mt-6">
                In subsequent years, hardware is not recharged unless new tablets are required. The subscription remains a single predictable line item.
              </div>
            </div>
          ...
          <div className="text-center mt-16">
            <div className="grid md:grid-cols-3 gap-8 max-w-4xl mx-auto">
              <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
                ...
                <h4 className="text-lg font-semibold text-white mb-2">Unlimited Data Included</h4>
                <p className="text-gray-300 text-sm">We cover tablet data costs for seamless operation.</p>
              </div>
              <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
                <CheckCircleIcon className="w-12 h-12 text-green-400 mx-auto mb-4" />
                <h4 className="text-lg font-semibold text-white mb-2">On-site Training</h4>
                <p className="text-gray-300 text-sm">Our team trains guards and management on-site.</p>
              </div>
              <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
                <CheckCircleIcon className="w-12 h-12 text-green-400 mx-auto mb-4" />
                <h4 className="text-lg font-semibold text-white mb-2">Support & Updates</h4>
                <p className="text-gray-300 text-sm">Continuous support, maintenance and improvements.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <Modal isOpen={isDemoOpen} onClose={() => setIsDemoOpen(false)} size="md" title="Request a Demo" align="start">
        <form onSubmit={submitDemoRequest} className="bg-white rounded-2xl p-6 border border-neutral-200">
