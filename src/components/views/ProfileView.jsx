...
export const ProfileView = () => {
  const { user, updatePersonalInfo, updateResidencyInfo, updateGuardInfo, submitResidencyRequest, residencyRequests, addNotification, allUsers, setAllUsers, setUser } = useAuth();
  const [isEditMode, setIsEditMode] = useState(false);
  const [isResidencyRequestModalOpen, setIsResidencyRequestModalOpen] = useState(false);
  ...
  const viewingResident = user.role === 'admin' && user.viewingResident ? user.viewingResident : null;
  const profileUser = viewingResident || user;

  // Permission toggle for admin
  const [canEdit, setCanEdit] = useState(viewingResident ? viewingResident.canEdit : false);
  ...
  const handleSave = () => {
    if (profileUser.role === 'resident') {
      updatePersonalInfo(profileUser.id, editForm.personalInfo, editForm.email, editForm.phone);
    } else if (profileUser.role === 'guard') {
  ...
  const handleSubmitResidencyRequest = (e) => {
    ...
    const request = {
      ...
      apartment: profileUser.residencyInfo?.apartmentNumber || profileUser.apartmentNumber
    };
  ...
  const handleRequestSubmit = (e) => {
    ...
    addNotification({
      ...
      data: { residentId: profileUser.id, title: requestTitle, details: requestDetails }
    });
    setRequestTitle('');
    setRequestDetails('');
      ...
      return (
        <div className="space-y-4">
          <div className="space-y-3">
            <label className="block text-sm font-medium">Full Name <span className="text-xs text-neutral-400">(Contact admin to change)</span></label>
            <Input value={profileUser.name} readOnly className="bg-neutral-100 cursor-not-allowed" />
          </div>
          <div className="space-y-3">
          ...
          <div className="space-y-3">
            ...
            <Input
              ...
              onChange={e => setEditForm(f => ({
                ...f,
                personalInfo: { ...f.personalInfo, nationality: e.target.value }
              }))}
            />
          </div>
          <div className="space-y-3">
            ...
            <Input
              ...
              onChange={e => setEditForm(f => ({
                ...f,
                personalInfo: { ...f.personalInfo, emergencyContactName: e.target.value }
              }))}
            />
          </div>
          <div className="space-y-3">
            ...
            <Input
              ...
              onChange={e => setEditForm(f => ({
                ...f,
                personalInfo: { ...f.personalInfo, emergencyContact: e.target.value }
              }))}
            />
          </div>
          <div className="space-y-3">
            <label className="block text-sm font-medium">Emergency Contact Relation</label>
            <Input
              value={editForm.personalInfo?.emergencyContactRelation || ''}
              onChange={e => setEditForm(f => ({
                ...f,
                personalInfo: { ...f.personalInfo, emergencyContactRelation: e.target.value }
              }))}
            />
          </div>
        </div>
      ...
      <Card className="glass-effect rounded-2xl shadow-soft border-0">
        <CardHeader className="pb-4">
          <div className="flex items-center gap-4">
            <div className={`w-16 h-16 bg-gradient-to-br ${getRoleColor(profileUser.role)} rounded-2xl flex items-center justify-center shadow-soft`}>
            ...
            <div>
              <h4 className="text-xl font-bold text-neutral-800">{profileUser.name}</h4>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-neutral-600">{profileUser.email}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <PhoneIcon className="w-4 h-4 text-neutral-400" />
                    <span className="text-neutral-600">{profileUser.phone || 'Not specified'}</span>
                  </div>
                </div>
              </div>
              <div className="space-y-4">
                <h5 className="font-semibold text-neutral-800 border-b pb-2">{profileUser.role === 'resident' ? 'Emergency Contact' : 'Account Information'}</h5>
                <div className="space-y-3">
                  {profileUser.role === 'resident' ? (
                    <>
                      <div className="flex items-center gap-3">
                        <UserIcon className="w-4 h-4 text-neutral-400" />
                        <span className="text-neutral-600">{profileUser.personalInfo?.emergencyContactName || 'Not specified'}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <PhoneIcon className="w-4 h-4 text-neutral-400" />
                        <span className="text-neutral-600">{profileUser.personalInfo?.emergencyContact || 'Not specified'}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-neutral-600">{profileUser.personalInfo?.emergencyContactRelation || 'Not specified'}</span>
                      </div>
                    </>
                  ) : (
                    <>
      ...
      {profileUser.role === 'resident' && (
        <>
          <Card className="glass-effect rounded-2xl shadow-soft border-0">
            <CardHeader className="pb-4">
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-bold font-display text-neutral-800">Residency Information</h3>
                <div className="flex gap-2">
                  <Button 
                  ...
                  {user.role === 'admin' && viewingResident && (
                    <Button onClick={handleTogglePermission} className={canEdit ? 'button-secondary' : 'button-primary'}>
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="text-xl font-bold text-neutral-800">
                    {profileUser.residencyInfo?.apartmentNumber || profileUser.apartmentNumber}
                  </h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <h5 className="font-semibold text-neutral-800 border-b pb-2">Property Details</h5>
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                ...
                <div className="space-y-4">
                  <h5 className="font-semibold text-neutral-800 border-b pb-2">Lease Information</h5>
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      <CalendarIcon className="w-4 h-4 text-neutral-400" />
                      <span className="text-neutral-600">
                        {profileUser.residencyInfo?.leaseStartDate ? new Date(profileUser.residencyInfo.leaseStartDate).toLocaleDateString() : 'N/A'}
                      </span>
                    </div>
                    <div className="flex items-center gap-3">
                      <CalendarIcon className="w-4 h-4 text-neutral-400" />
                      <span className="text-neutral-600">
                        {profileUser.residencyInfo?.leaseEndDate ? new Date(profileUser.residencyInfo.leaseEndDate).toLocaleDateString() : 'N/A'}
                      </span>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-neutral-600">
                        ₦{profileUser.residencyInfo?.monthlyRent?.toLocaleString() || 'N/A'} /month
                      </span>
                    </div>
                    <div className="flex items-center gap-3">
                      <CheckCircleIcon className="w-4 h-4 text-green-500" />
          ...
          {residencyRequests.filter(req => req.residentId === profileUser.id).length > 0 && (
            <Card className="glass-effect rounded-2xl shadow-soft border-0">
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-bold font-display text-neutral-800">Recent Requests</h3>
                  <Badge className="bg-blue-100 text-blue-700 border-blue-200">
                    {residencyRequests.filter(req => req.residentId === profileUser.id).length} total
                  </Badge>
                </div>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {residencyRequests
                    ...
                    .map((request) => (
                      <div key={request.id} className="flex items-center justify-between p-3 bg-white/50 rounded-xl border">
                        <div className="flex items-center gap-3">
                ...
                {residencyRequests.filter(req => req.residentId === profileUser.id).length > 3 && (
                  <div className="mt-3 pt-3 border-t border-neutral-200">
                    <p className="text-xs text-neutral-500 text-center">
                      +{residencyRequests.filter(req => req.residentId === profileUser.id).length - 3} more requests
                    </p>
                  </div>
                )}
              </CardContent>
        ...
        <Modal isOpen={isRequestModalOpen} onClose={() => setIsRequestModalOpen(false)}>
          <form onSubmit={handleRequestSubmit} className="space-y-4 p-4">
            ...
            <div className="flex justify-end gap-2">
              <Button type="button" onClick={() => setIsRequestModalOpen(false)} className="button-secondary">Cancel</Button>
