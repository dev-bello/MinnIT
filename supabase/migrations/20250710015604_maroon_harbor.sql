...
ALTER TABLE estates ADD COLUMN IF NOT EXISTS subdomain text;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conname = 'estates_subdomain_key'
    ) THEN
        ALTER TABLE estates ADD CONSTRAINT estates_subdomain_key UNIQUE (subdomain);
    END IF;
END $$;
...
DROP POLICY IF EXISTS "Developers can manage their estate subscriptions" ON subscriptions;
DROP POLICY IF EXISTS "Estate admins can view their estate subscriptions" ON subscriptions;
DROP POLICY IF EXISTS "Developers can view their estate usage" ON subscription_usage;
DROP POLICY IF EXISTS "Estate admins can view their estate usage" ON subscription_usage;
DROP POLICY IF EXISTS "Developers can manage their estate settings" ON estate_settings;
DROP POLICY IF EXISTS "Estate admins can view their estate settings" ON estate_settings;
DROP POLICY IF EXISTS "Developers can view their estate audit logs" ON audit_logs;
DROP POLICY IF EXISTS "Estate admins can view their estate audit logs" ON audit_logs;
...
DROP TRIGGER IF EXISTS update_estate_settings_updated_at ON estate_settings;
...
BEGIN
    ...
    IF sample_estate_id IS NOT NULL THEN
        ...
        -- Create sample subscription record (with conflict handling)
        ...
        SELECT 
          ...
          false
        WHERE NOT EXISTS (
          SELECT 1 FROM subscriptions WHERE estate_id = sample_estate_id
        );
        
        -- Create default estate settings with conflict handling
        INSERT INTO estate_settings (estate_id, setting_key, setting_value) 
        SELECT sample_estate_id, 'visitor_settings', '{"allow_self_registration": false, "require_approval": true, "max_visitors_per_day": 10}'::jsonb
        WHERE NOT EXISTS (
          SELECT 1 FROM estate_settings 
          WHERE estate_id = sample_estate_id AND setting_key = 'visitor_settings'
        );
        
        INSERT INTO estate_settings (estate_id, setting_key, setting_value) 
        SELECT sample_estate_id, 'notification_settings', '{"email_notifications": true, "sms_notifications": false, "push_notifications": true}'::jsonb
        WHERE NOT EXISTS (
          SELECT 1 FROM estate_settings 
          WHERE estate_id = sample_estate_id AND setting_key = 'notification_settings'
        );
        
        INSERT INTO estate_settings (estate_id, setting_key, setting_value) 
        SELECT sample_estate_id, 'security_settings', '{"require_photo_id": true, "visitor_photo_required": false, "background_check": false}'::jsonb
        WHERE NOT EXISTS (
          SELECT 1 FROM estate_settings 
          WHERE estate_id = sample_estate_id AND setting_key = 'security_settings'
        );
        
    END IF;
END $$;

-- Insert default estate settings for existing estates (with conflict handling)
INSERT INTO estate_settings (estate_id, setting_key, setting_value)
SELECT 
  id,
  'default_settings',
